class Admin::MembersController < Admin::Base
  ...
  def update
    ...
    @member.assign_attributes(member_params) #フォームからのデータをセット
    if @member.save
      redirect_to [:admin, @member], notice: "会員情報を更新しました" #saveが成功するとTrueを返しDBを更新、noticeを表示
    else
  ...
  def destroy
    ...
    redirect_to :admin_members, notice: "会員を削除しました" #メンバー一覧ページにリダイレクト。noticeを表示させる
  end
# ストロングパラメータ
